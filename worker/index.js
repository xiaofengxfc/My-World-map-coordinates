  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (method === 'GET' && path === '/api/locations') {
        ...
        if (search) {
          conditions.push('(name LIKE ? OR description LIKE ? OR x || \' \' || y || \' \' || z LIKE ?)')
          const q = `%${search}%`
          params.push(q, q, q)
        }
