...
import type { Location, LocationForm, Category } from '../types'
...
export function useCoords() {
  ...
  async function loadLocations() {
    ...
    try {
      ...
      locations.value = await api<Location[]>(`/locations?${params}`)
    } catch (err) {
  ...
  async function loadCategories() {
    try { categories.value = await api<Category[]>('/categories') } catch { categories.value = [] }
  }
